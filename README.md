...
- приветствует командой `/start` и показывает клавиатуру выбора голосов (список из ElevenLabs);
- принимает аудио/voice, сохраняет их локально;
- умеет присылать сохраненные файлы по команде `/send` и показывать список `/files`;
